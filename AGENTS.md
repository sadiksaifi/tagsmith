...
- Config shape changes must update parser validation, `schema/v1.json`, the init template, the human docs under `docs/docs/**`, and the agent manual at `docs/public/llms.txt` together. The same co-update rule applies when changing release planning, CLI parsing, or CLI command orchestration, because both the human docs site and the agent manual mirror all user-visible behavior. The docs-drift CI guard enforces the `docs/public/llms.txt` half on the watched paths; the human docs co-update is enforced by review.
