...
- Config shape changes must update parser validation, `schema/v1.json`, the init template, and `docs/llms.txt` together. The same co-update rule applies when changing release planning, CLI parsing, or CLI command orchestration, because the agent-facing manual at `docs/llms.txt` mirrors all user-visible behavior. CI enforces this on the watched paths.
